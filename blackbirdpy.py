...
<style type='text/css'>.bbpBox{id} {{{bbpBoxCss}background:url({profileBackgroundImage}) #{profileBackgroundColor};padding:20px;;width:80%;margin-right:auto;margin-left:auto}} p.bbpTweet{{background:#fff;padding:10px 12px 10px 12px;margin:0;min-height:48px;color:#000;font-size:125% !important;line-height:1.3;-moz-border-radius:5px;-webkit-border-radius:5px}}
p.bbpTweet a{{color:blue;text-decoration:none}} p.bbpTweet a:hover{{text-decoration:underline}} p.bbpTweet span.metadata{{display:block;width:100%;clear:both;margin-top:8px;padding-top:12px;height:40px;border-top:1px solid #fff;border-top:1px solid #e6e6e6;font-family:Sans-serif;font-size:80%;line-height:1.1}} p.bbpTweet span.metadata span.author{{}} p.bbpTweet span.metadata span.author span.realName{{color:#888;font-size:75%}} p.bbpTweet span.metadata span.author img{{float:left;margin:0 7px 0 0px;width:36px;height:36px}} p.bbpTweet span.timestamp{{font-family:Sans-serif;font-size:50%;display:block;padding-top:0.5em;text-align:right}}</style>
<div class='bbpBox{id}'><p class='bbpTweet'>{tweetText}<span class='timestamp'><a title='{timeStamp}' href='{tweetURL}'>{easyTimeStamp}</a></span><span class='metadata'><span class='author'><a href='http://twitter.com/{screenName}'><img src='{profilePic}' /></a><a href='http://twitter.com/{screenName}'><strong>@{screenName}</strong></a><br/><span class='realName'>{realName}</span></span></span></p></div>
<!-- end of tweet -->'''
